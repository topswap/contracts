...
// Dependency file: contracts/BSCswapPair.sol
...
// Dependency file: contracts/libraries/BSCswapOracleLibrary.sol
...
library BSCswapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = BSCswapPair(pair).price0CumulativeLast();
        price1Cumulative = BSCswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = BSCswapPair(pair).getReserves();
...
// Dependency file: contracts/libraries/FixedPoint.sol
...
// import '../libraries/BSCswapOracleLibrary.sol';
...
contract ExampleOracleSimple {
    ...
    constructor(address factory, address tokenA, address tokenB) public {
        IBSCswapPair _pair = IBSCswapPair(BSCswapLibrary.pairFor(factory, tokenA, tokenB));
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
    ...
    function update() external {
        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) =
            BSCswapOracleLibrary.currentCumulativePrices(address(pair));
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // overflow is desired
